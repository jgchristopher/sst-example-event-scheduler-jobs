...
export function Job1Stack({ app, stack }: StackContext) {
  ...
  const invokeLambdaPolicy = new iam.Policy(
    stack,
    `${app.logicalPrefixedName(JOB_NAME)}-InvokeLambdaPolicy`,
    {
  ...
  new cdk.CfnResource(stack, `${app.logicalPrefixedName(JOB_NAME)}-Schedule`, {
    type: "AWS::Scheduler::Schedule",
    properties: {
      GroupName: SCHEDULER_GROUP_NAME,
      Name: `${app.logicalPrefixedName(JOB_NAME)}-Schedule`,
      State: "DISABLED",
